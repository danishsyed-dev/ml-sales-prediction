    ...
    try:
        if aws_available:
            ...
            if os.path.exists(file_path):
                ...
                main_logger.error(f"Local file not found: {file_path}")
                return None
        ...
        main_logger.error(f"failed to load file: {e}. return None")
        return None
...
def s3_load_to_temp_file(file_path):
    ...
    main_logger.info(f"... loading {file_path} from S3 or local...")
    try:
        ...
        main_logger.error(f"Error downloading {file_path}: {e}")
        return None
