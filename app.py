import os
import boto3 # type: ignore  # Only needed for AWS Lambda, not local
import json
import time
import math
import warnings
...
if os.environ.get('ENV') == 'local':
    main_logger.info('Running in local mode: S3 and STS checks are skipped.')
    ...
    try:
        if os.environ.get('ENV') == 'local':
            ...
            if os.path.exists(X_TEST_PATH):
                ...
                main_logger.error(f'Local x_test file not found: {X_TEST_PATH}')
        else:
    ...
    try:
        if os.environ.get('ENV') == 'local':
            ...
            if os.path.exists(PREPROCESSOR_PATH):
                ...
                main_logger.error(f'Local preprocessor file not found: {PREPROCESSOR_PATH}')
        else:
        ...
        main_logger.error(f'Error loading preprocessor: {e}')
